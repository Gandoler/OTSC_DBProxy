...
```
        ...
        `---------------------'   `-------'                                                     
                                       ...
                                       |                                                        
,-------.  ,-----------.   ,---------------------.   ,----------------.   ,-----------.   ,----.
|Friends|  |TG подписка|   |Восстановление пароля|   |Нейро заполнение|   |Регистрация|   |Auth|
|-------|  |-----------|   |---------------------|   |----------------|   |-----------|   |----|
`-------'  `-----------'   `---------------------'   `----------------'   `-----------'   `----'
                            ...
                            ,------------.   ,---------.   ,--.                                 
                            |Telegram-бот|   |Email-бот|   |БД|                                 
                            |------------|   |---------|   |--|                                 
                            `------------'   `---------'   `--'                                 
                                   |              |                                             
                          ,---------------.    ,-----.                                          
                          |Telegram Client|    |Email|                                          
                          |---------------|    |-----|                                          
                          `---------------'    `-----'                                          
...
🧹 Инструменты для чистого кода в ASP.NET Core проекте
1. ✅ Линтер — StyleCop.Analyzers
📌 Что это такое:
Линтер анализирует исходный код на соответствие стандартам и правилам, выявляя нарушения стиля, плохие практики и потенциальные ошибки.
...
SA1600: отсутствует документация для публичного члена

SA1649: имя файла не совпадает с именем класса

🛠 Особенности:
Не все ошибки можно исправить автоматически — часть из них требует ручной правки. Также можно отключать ненужные правила через .editorconfig или stylecop.json.

2. 🖌 Код стайлер — .editorconfig
📌 Что это такое:
Файл .editorconfig содержит правила оформления кода — отступы, пробелы, переносы строк, порядок using, стиль скобок и т.д.

🗂 Где находится:
Файл создан в корне проекта, рядом с .sln.
...
🛠 Как работает:
Visual Studio и Rider автоматически применяют эти правила при написании и сохранении кода. Также ими пользуется dotnet format при автоформатировании.

3. 🛠 Стайлфиксеры — dotnet format
📌 Что это такое:
...
⚠ Особенности:
Некоторые ошибки (SA1600, SA1649 и т.д.) невозможно исправить автоматически.

Можно использовать --verify-no-changes для проверки нарушений в CI.

✅ Вывод
Использование StyleCop.Analyzers, .editorconfig и dotnet format обеспечивает:

единообразие стиля кода;

уменьшение количества ошибок и хаоса;

автоматическую проверку и частичное исправление;

соблюдение стандартов командной разработки.
